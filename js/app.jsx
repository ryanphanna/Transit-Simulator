(function (g) {
  ...
  const {
    ...
    routePerformanceSummary, routeBand, allocateFleet, driverHoursAvailable, maxDepotThroughput
  } = TS;
  const { useBanners, InfoTip, NumberStepper, MapToast, BandPill } = TS;
  ...
  function App(){
    ...
    const routeSummaries = useMemo(() => {
      ...
      return routes.map((route, index) => {
        ...
        const performance = (estimate && route.stops.length >= 2)
        ...
        return {
          ...
          band: route.stops.length >= 2 ? (performance?.band ?? null) : null,
    ...
    }, [routes, enrichedRoutes, routeEstimateMap, polylineFor, routePerformanceSummary, ROUTE_COLORS]);

    const activeRouteSummary = useMemo(() => routeSummaries.find(r => r.id === activeRouteId) || null, [routeSummaries, activeRouteId]);
    const networkBandLabel = useMemo(() => {
      const entries = routeSummaries
        .filter(summary => summary.score !== null && summary.score !== undefined)
        .map(summary => ({
          score: summary.score,
          weight: Number.isFinite(summary.estimate?.perDay) ? Math.max(0, summary.estimate.perDay) : null
        }));
      ...
      const avgScore = weightTotal > 0 ? (weightedSum / weightTotal) : (fallbackCount > 0 ? (fallbackSum / fallbackCount) : null);
      if(avgScore === null) return null;
      return routeBand(avgScore);
    }, [routeSummaries, routeBand]);
      ...
      <div className="relative min-h-screen w-full bg-gradient-to-b from-sky-50 via-emerald-50/40 to-white text-slate-900">
          ...
          <main className="mx-auto w-full max-w-screen-2xl px-6 py-6 md:px-10 lg:px-14">
            <div className="flex flex-col items-center text-center">
              ...
              <p className="mt-1 flex items-center justify-center gap-2 text-sm text-slate-700">
                <span>Network performance:</span>
                {networkBandLabel ? (
                  <BandPill band={networkBandLabel} size="sm" />
                ) : (
                  <span className="text-slate-400">—</span>
                )}
                <span>· Citizens appreciate every comfy ride.</span>
              </p>
            </div>
            ...
            <div className="grid min-h-[calc(100vh-8rem)] grid-cols-1 items-stretch gap-6 pt-6 pb-12 sm:px-2 lg:grid-cols-[320px_minmax(0,1fr)_340px]">
                  ...
                  <div className="rounded-xl border border-emerald-100/70 bg-emerald-50/60 p-3">
                    ...
                    <div className="mt-2 space-y-1">
                      <div className="flex items-center justify-between">
                        <span className="flex items-center gap-1">Network band<InfoTip text="Balance, connectivity, spacing, and route length." /></span>
                        {networkBandLabel ? <BandPill band={networkBandLabel} size="sm" /> : <span className="text-slate-400">—</span>}
                      </div>
              ...
              <section className="order-1 flex h-full min-h-[420px] min-w-0 flex-col gap-4 rounded-2xl border border-emerald-100/60 bg-white/85 p-4 shadow-sm lg:order-2">
                ...
                <div className="flex items-center gap-3 text-xs text-slate-600">
                  ...
                  {activeRouteSummary?.band ? (
                    <BandPill band={activeRouteSummary.band} size="sm" />
                  ) : (
                    <span className="text-slate-400">—</span>
                  )}
                </div>
                      ...
                      <div className="mt-2 max-h-28 space-y-2 overflow-auto pr-1">
                        {routeSummaries.map(summary => (
                          ...
                          >
                            ...
                            {summary.band ? (
                              <BandPill band={summary.band} size="sm" />
                            ) : (
                              <span className="text-slate-400">—</span>
                            )}
                          </div>
